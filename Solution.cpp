...
int getKthElement_findMedianSortedArrays(vector<int> &nums1, vector<int> &nums2, int k) {
    int m = nums1.size();
    int n = nums2.size();
    int index1 = 0, index2 = 0;
    // k represents the left num
    // if k==1 then the last one is what we need to get
    while (true) {
...
double Solution::findMedianSortedArrays(vector<int> &nums1, vector<int> &nums2) {
    ...
    int len = len1 + len2;
    if(len % 2 == 1) {
        return getKthElement_findMedianSortedArrays(nums1, nums2, (len+1) / 2);
    } else {
        return (getKthElement_findMedianSortedArrays(nums1, nums2, len / 2)
